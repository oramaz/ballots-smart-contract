...
describe('Ballots contract', function () {
	...
	describe('Withdrawing fee', function () {
		it('Should be reverted with a onlyOwner reason', async function () {
			await expect(contract.connect(addr1).withdrawFee()).to.be.revertedWith(
				"The sender doesn't have owner rights"
			);
		});

		it('Should be reverted with an zero fee amount reason', async function () {
			await expect(contract.connect(owner).withdrawFee()).to.be.revertedWith(
				'Fee is zero'
			);
		});
		...
		it("Should withdraw the ballot's fee", async function () {
			...
			await contract.connect(owner).withdrawFee();
